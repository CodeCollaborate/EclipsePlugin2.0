...
public class NetworkProjectGrantPermissions extends AbstractNetworkExtension implements IProjectGrantPermissionsExtension {
		...
		Request grantPermReq = (new ProjectGrantPermissionsRequest(projectID, grantUsername, permission))
				.getRequest(response -> {
					...
					if (status == 200) {
						...
						Set<ICoreExtension> extensions = extMgr.getExtensions(NetworkExtensionIDs.PROJECT_GRANT_PERMISSIONS_REQUEST_ID, IProjectGrantPermissionsResponse.class);
