...
import org.code.toboggan.filesystem.extensions.file.FSFileRename;
import org.code.toboggan.modelmgr.extensions.file.ModelMgrFileRename;
import org.code.toboggan.modelmgr.integration.AbstractTest;
import org.code.toboggan.network.request.extensions.NetworkExtensionManager;
import org.code.toboggan.network.request.extensions.file.NetworkFileRename;
...
public class TestFileRename extends AbstractTest {
	@Before
	...
	public void testSuccessfulPath() throws InterruptedException {
		...
		for (File fData : testFiles) {
			...
			Path newFilePath = fData.getRelativePath().resolve(fData.getFilename() + "renamed");
			IFile newIFile = testIProject.getFile(newFilePath.toString());

			fData.setFilename(fData.getFilename() + "renamed");

			FileRename fRename = new FileRename(APIExtensionManager.getInstance(), fData.getFileID(),
			...
			Mockito.verify(wsMgr).sendAuthenticatedRequest(Mockito.argThat(new ArgumentMatcher<Request>() {
				...
				public boolean matches(Request argument) {
					if (!(argument.data instanceof FileRenameRequest)) {
						return false;
					}
					
					req = argument;

					FileRenameRequest fileRenameReq = (FileRenameRequest) req.data;
					return fileRenameReq.getFileID() == fData.getFileID()
							&& fileRenameReq.getNewName().equals(fData.getFilename());
				}
			}));
