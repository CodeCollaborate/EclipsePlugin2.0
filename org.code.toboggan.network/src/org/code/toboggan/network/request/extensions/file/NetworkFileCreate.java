...
public class NetworkFileCreate extends AbstractNetworkExtension implements IFileCreateExtension {
	...
	public void fileCreated(String name, Path fileLocation, long projectID, byte[] fileBytes) {
		...
		String contents = new String(fileBytes).replace("\r\n", "\n");
		// Make request
		Request createFileReq = new FileCreateRequest(name, projectRelativePath, projectID, contents.getBytes())
				.getRequest(response -> {
					int status = response.getStatus();
					if (status == 200) {
						long fileID = ((FileCreateResponse) response.getData()).fileID;

						Set<ICoreExtension> extensions = extMgr
								.getExtensions(NetworkExtensionIDs.FILE_CREATE_REQUEST_ID, IFileCreateResponse.class);
						for (ICoreExtension e : extensions) {
							IFileCreateResponse p = (IFileCreateResponse) e;
							p.fileCreated(fileID, name, fileLocation, projectRelativePath, projectID, contents);
						}
	...
	private void handleCreateError(String name, Path fileLocation, long projectID, byte[] fileBytes) {
		...
		Set<ICoreExtension> extensions = extMgr.getExtensions(NetworkExtensionIDs.FILE_CREATE_REQUEST_ID,
				IFileCreateResponse.class);
		for (ICoreExtension e : extensions) {
			IFileCreateResponse p = (IFileCreateResponse) e;
	...
	private IRequestSendErrorHandler getCreateSendHandler(String name, Path fileLocation, long projectID,
			byte[] fileBytes) {
