...
public class TestProjectLookup extends AbstractTest {
	...
	public void testSuccessfulPath() throws InterruptedException {
		// Run API call
		ProjectLookup pLookup = new ProjectLookup(APIExtensionManager.getInstance(),
				Arrays.asList(new Long[] { testProject.getProjectID() }));
		pLookup.execute();
		...
		Assert.assertEquals("Inserted Project was not equal to testProject", ss.getProject(testProject.getProjectID()),
				testProject);
	}
}
