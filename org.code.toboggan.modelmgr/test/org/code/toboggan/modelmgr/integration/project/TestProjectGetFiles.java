...
import org.code.toboggan.network.request.extensions.project.NetworkProjectGetFiles;
...
public class TestProjectGetFiles extends AbstractTest {
	...
	public void testSuccessfulPath() throws InterruptedException {
		...
		for (File fData : testFiles) {
			...
			Assert.assertNotNull("File could not be found by ID in project metadata", ss.getProject(testProject.getProjectID()).getFile(fData.getFileID()));
			Assert.assertNotNull("File could not be found by filepath in project metadata", ss.getProject(testProject.getProjectID()).getFile(filePath));
		}
	}
}
