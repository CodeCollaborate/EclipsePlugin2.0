...
import org.code.toboggan.filesystem.extensions.file.FSFilePull;
	...
	public void testSuccessfulPath() throws InterruptedException, IOException, CoreException {
		...
		for (int i = 0; i < testFiles.length; i++) {
			...
			if (fData.getFileID() == 1) {
				patches = new String[] { String.format("v1:\n0:-%d:%s", fContent.length(), fContent) };
			}
			IResponseData data = new FilePullResponse(fContent.getBytes(), patches);
