...
public class NetworkFilePullDiffSendChanges extends AbstractNetworkExtension
		implements IFilePullDiffSendChangesExtension {
	...
	public void filePulled(long fileID) {
		...
		// Set<ICoreExtension> extensions =
		// extMgr.getExtensions(NetworkExtensionIDs.FILE_PULL_DIFF_SEND_CHANGES_REQUEST_ID,
		// IFilePullDiffSendChangesResponse.class);
		// for (ICoreExtension e : extensions) {
		// IFilePullDiffSendChangesResponse p =
		// (IFilePullDiffSendChangesResponse) e;
		...
		logger.debug("PullDiffSendChanges using shadow document");

		Set<ICoreExtension> extensions = extMgr.getExtensions(
				NetworkExtensionIDs.FILE_PULL_DIFF_SEND_CHANGES_REQUEST_ID, IFilePullDiffSendChangesResponse.class);
		for (ICoreExtension e : extensions) {
			IFilePullDiffSendChangesResponse p = (IFilePullDiffSendChangesResponse) e;
			p.filePulled(fileID, null, null);
		}
	}

	// private void handlePullError(long fileID) {
	// logger.error(String.format("Failed to pull file on server: [%d]",
	// fileID));
	// Set<ICoreExtension> extensions =
	// extMgr.getExtensions(NetworkExtensionIDs.FILE_PULL_DIFF_SEND_CHANGES_REQUEST_ID,
	// IFilePullDiffSendChangesResponse.class);
	// for (ICoreExtension e : extensions) {
	// IFilePullDiffSendChangesResponse p = (IFilePullDiffSendChangesResponse)
	// e;
