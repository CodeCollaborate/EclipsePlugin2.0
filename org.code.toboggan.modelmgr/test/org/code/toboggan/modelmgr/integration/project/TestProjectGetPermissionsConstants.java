...
import org.code.toboggan.modelmgr.extensions.project.ModelMgrProjectGetPermissionConstants;
import org.code.toboggan.modelmgr.integration.AbstractTest;
import org.code.toboggan.network.request.extensions.NetworkExtensionManager;
import org.code.toboggan.network.request.extensions.project.NetworkProjectGetPermissionConstants;
...
public class TestProjectGetPermissionsConstants extends AbstractTest {
	...
	public void testSuccessfulPath() throws InterruptedException {
		...
		Map<String, Byte> permissionsConstants = new HashMap<>();
		permissionsConstants.put("key1", (byte) 5);
		permissionsConstants.put("key2", (byte) 7);
		...
		Assert.assertNotNull("Could not find key1 by value", ss.getPermissionConstants().inverse().get((byte) 5));
		Assert.assertNotNull("Could not find value for key2", ss.getPermissionConstants().get("key2"));
		Assert.assertNotNull("Could not find key2 by value", ss.getPermissionConstants().inverse().get((byte) 7));
	}
}
