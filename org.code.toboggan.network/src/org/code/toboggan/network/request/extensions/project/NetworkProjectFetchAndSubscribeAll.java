...
public class NetworkProjectFetchAndSubscribeAll extends AbstractNetworkExtension implements IProjectFetchSubscribeAllExtension {
	...
	public void projectFetchSubscribeAllOccurred(List<Long> subscribedIds) {
		...
		Request getProjectsRequest = new UserProjectsRequest().getRequest(response -> {
			...
			if (status == 200) {
				...
				Set<ICoreExtension> extensions = extMgr.getExtensions(NetworkExtensionIDs.PROJECT_FETCH_SUBSCRIBE_ALL_REQUEST_ID, IProjectFetchAndSubscribeAllResponse.class);
				...
				Set<Long> returnedProjectIDs = new HashSet<>(Arrays.asList(projects.stream().map(p -> p.getProjectID()).toArray(Long[]::new)));
				
				for (long id : subscribedIds) {
					// Only attempt to subscribe if the project ID was returned in the User.Projects request; if it is not in the returned projects, 
					// it has been deleted, or we no longer have permissions to it.
